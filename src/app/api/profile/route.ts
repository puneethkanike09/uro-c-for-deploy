  ...
  try {
    ...
    const convertFileKeyToUrl = (
      ...
      if (!fileKey || fileKey === "https://example.com" || fileKey === "") {
        console.log(
          "Profile API PUT: File key is empty or placeholder, returning null"
        );
        return null;
      }
      ...
      if (fileKey.startsWith("http")) {
        console.log(
          "Profile API PUT: File key is already a URL, returning as is"
        );
        return fileKey;
      }
      ...
      if (fileKey.startsWith("local-")) {
        console.log(
          "Profile API PUT: Mock file key detected, returning placeholder URL"
        );
