...
import { uploadFileToS3, generateFileKey, isValidFileType, isValidFileSize } from '@/lib/s3';
  ...
  try {
    ...
    if (!isValidFileType(file.name, fileType)) {
      const allowedTypes = fileType === 'avatar' 
        ...
        : 'PDF, DOC, and DOCX';
      console.log('Upload API: Invalid file type:', file.name, 'for type:', fileType);
      return NextResponse.json(
    ...
    if (!process.env.AWS_ACCESS_KEY_ID || !process.env.AWS_SECRET_ACCESS_KEY) {
      console.log('Upload API: S3 not configured, using local storage fallback');
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!fileKey.includes(decoded.userId)) {
      console.log('Upload API DELETE: Security check failed - file key does not contain user ID');
      return NextResponse.json(
